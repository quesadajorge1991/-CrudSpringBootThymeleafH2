...
public class WebSecurityConfiguration {
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		return http.headers().frameOptions().disable().and().csrf().disable()
				...
				.requestMatchers("/", "/index", "/profile").authenticated()
				.requestMatchers(new AntPathRequestMatcher("/h2-console/**")).hasAnyRole("ADMIN")
				.requestMatchers("/user/**").hasAnyRole("ADMIN")
				.requestMatchers("/employe/**").hasAnyRole("EMPLOYE")
